...
parser.add_argument('--fileSelectedCell', help = 'list of cell-ID to select')
parser.add_argument('--out', help = 'Fastq file outptu name, without _Rx.fastq', type = str, default = 'None')
...
listcells = args.fileSelectedCell
hd = args.cbchd
outfile = args.out

if not os.path.isfile(listcells):
    sys.exit('file with listed cells not found')

listcells = read_csv(listcells, header = None)
listcells = listcells[0].astype(str).tolist()
...
#### Do the job ####
if outfile != 'None':
    foutR1 = open(outfile + '_R1.fastq', 'w')
    foutR2 = open(outfile + '_R2.fastq', 'w')
else:
    foutR1 = open(fqr + '_' + '-'.join(listcells) + '_R1.fastq', 'w')
    foutR2 = open(fqr + '_' + '-'.join(listcells) + '_R2.fastq', 'w')
