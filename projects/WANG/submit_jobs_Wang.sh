...
echo "skipping GSM3449620_N14_cat for now -- but restart for whole procedure!!"
#lib=GSM3449620_N14_cat
#sbatch  --output=slurm-${lib}-%x.%j.out --job-name=${lib} --time=48:00:00  --mem=50G --export=ALL,general_parameter_filepath="${general_parameter_filepath}",run_parameter_filepath="${run_parameter_filepath}",lib="$lib" ${script_path}/L_TS_submit_vasaplate_map_LOCAL.sh
