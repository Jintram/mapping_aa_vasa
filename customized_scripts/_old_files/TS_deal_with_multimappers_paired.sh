...
}' | samtools view -Sb > ${inbam%.bam}.multimappers.bam

bedtools bamtobed -i ${inbam%.bam}.multimappers.bam | bedtools sort > ${inbam%.bam}.multimappers.bed
# locally: 
# ${p2b}bedtools bamtobed -i ${inbam%.bam}.multimappers.bam | ${p2b}bedtools sort > ${inbam%.bam}.multimappers.bed

if [ $stranded == "y" ]
then
    # to run locally, add /Users/m.wehrens/Software_custom/bedtools2/bin/
    bedtools intersect -a ${inbam%.bam}.multimappers.bed -b $refBED -wa -wb | awk 'BEGIN {OFS="\t"; w="T"} {
