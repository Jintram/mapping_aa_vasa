...
df['attribute'] = [{f.rsplit()[0]: f.rsplit()[1].replace('"', '') for f in df.loc[idx, 'attribute'].rsplit(';') if
                    len(f.rsplit()) == 2} for idx in df.index]
...
if 'gene_biotype' in df.columns:
    df['gene_biotype'] = [''.join([y.capitalize() for y in x.rsplit('_')]) if len(x.rsplit('_')) > 1 else x for x in
                          df['gene_biotype']]
else:
    df['gene_biotype'] = 'undet'
...
fgdf.to_csv(output + '_genes.bed', sep='\t', header=False, index=None)
...
def findGeneExonIntron(xdfg):
    ...
    exons_nts = sorted(set([i for idx in xdf_f['exon'].index for i in
                            range(xdf_f['exon'].loc[idx, 'start'], xdf_f['exon'].loc[idx, 'end'])]))
...
for mdf in p.imap_unordered(findGeneExonIntron, [xdf[g] for g in xdf.keys()]):
    edf = edf.append(mdf[0])
    idf = idf.append(mdf[1])
...
uidf = [{'chr': i[0], 'start': i[1], 'end': i[2], 'strand': i[3],
         'gene_fullname': '-'.join(sorted(np.array(df_i['gene_fullname'])))} for i, df_i in
        idf.groupby(['chr', 'start', 'end', 'strand'])]
uidf = pd.DataFrame(uidf)[['chr', 'start', 'end', 'strand', 'gene_fullname']]

uedf = [{'chr': i[0], 'start': i[1], 'end': i[2], 'strand': i[3],
         'gene_fullname': '-'.join(sorted(np.array(df_i['gene_fullname'])))} for i, df_i in
        edf.groupby(['chr', 'start', 'end', 'strand'])]
...
# print out results
uidf.to_csv(output + '_introns.bed', sep='\t', header=False, index=None)
uedf.to_csv(output + '_exons.bed', sep='\t', header=False, index=None)
